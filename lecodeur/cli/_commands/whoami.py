"""``lecodeur whoami`` — the smallest identity probe.

Reports the agent's identity as declared in ``culture.yaml``: its nick
(``suffix``), the backend it runs on, and the served model — plus the package
version. Read-only; touches nothing but its own ``culture.yaml``.
"""
...
_FALLBACK_NICK = "lecodeur"


def _find_culture_yaml() -> Path | None:
    """Locate lecodeur's own ``culture.yaml`` by walking up from this module.

    The identity must be lecodeur's, not whatever ``culture.yaml`` happens to
    sit in the user's current working directory. In an editable / source
    install, walking up from ``__file__`` finds the repo root; in a wheel
    install no ``culture.yaml`` ships alongside the package and the caller
...
def _read_agent_fields() -> dict[str, str]:
    """Return ``suffix``/``backend``/``model`` from the first agent block.

    Parsed without a YAML dependency to keep lecodeur's runtime deps empty.
    Reads top-level ``key: value`` lines within the first agent entry; anything
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "whoami",
        help="Report lecodeur's nick, version, backend, and served model.",
    )
