"""Smoke tests for the lecodeur CLI entry point and its three verbs."""
...
from lecodeur.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: lecodeur" in capsys.readouterr().out
...
def test_explain_backend(capsys: pytest.CaptureFixture[str]) -> None:
    # Acceptance criterion: `lecodeur explain backend` works.
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "lecodeur whoami" in payload["markdown"]
