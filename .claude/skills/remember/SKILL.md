...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope lecodeur
...
scope:** the default personal scope is **private** (`--scope lecodeur
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `lecodeur`). Private records are served only to a recall in the same
